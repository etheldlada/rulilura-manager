...
const SONGS_CATALOG = [
  ...
  { no:87, name:'かく乱の歌「三つの変奏曲」', maxRank:4,
    effects: {
      1: '気力×2回消耗。本人あるいはペア英雄に対する偵察チェックに-30（30分）',
      '2A': '上記をペア英雄の装甲に及ぼす',
      '2B': '対象の命中値と回避修正に-10（30分）',
      3: '対象の抵抗修正に-20。抵抗チェックに成功した場合は-10',
      4: '2Bの効果を-20に強化',
    }},
  { no:88, name:'調和と協調の歌「妖精の森のソネット」', maxRank:3,
    effects: {
      1: '気力×2回消耗。可聴範囲内の小動物などに簡単な任務を遂行させる（最大1日）',
      2: '可聴範囲内のち生の低い敵は抵抗チェックに失敗すると1D10ターン戦闘行為を中止',
      3: '可聴範囲内の石や木、草などを操ることができる（30分）攻撃はできない',
    }},
  { no:89, name:'身護りの歌「百鐘組曲」', maxRank:4,
    effects: {
      1: '気力並びに肉体×1回消耗。歌姫本人は追加の防御値+5、命中値に-30（30分）',
      2: '上記を同じマスにいる味方全員に',
      3: '敵が歌姫に攻撃する場合、抵抗チェックに失敗すると攻撃を中止',
      4: 'ランク１の防御値が歌姫レベルに',
    }},
  { no:90, name:'リンクの歌Ⅱ「輝きのうた」', maxRank:4,
    effects: {
      1: '気力×1回消耗。ペア英雄の非戦闘スキル一つを「10+絆レベル」上昇(30分)',
      2: '自分に対して上記効果を適用',
      3: 'ペア英雄がMPを消費で気力消耗ゲージの印を消去',
      4: '以降30分以内に使う歌術で、ペア英雄を対象にする場合、修正に+絆レベル',
    }},
  { no:91, name:'呪いの歌(禁忌)', maxRank:4,
    effects: {
      1: '気力×4回消耗。対象のD%判定に-20ペナ',
      2: '対象ペアのすべての2d10判定に-3ペナ',
      '3A': '対象を行動不能にする',
      '3B': '対象に指定した特定の行動を行わせる',
      4: '対象を2D10*4日かけて、緩やかな死に至らしめる',
    }},
  { no:92, name:'幻影の歌(禁忌)', maxRank:5,
    effects: {
      1: '気力×1回消耗。周囲1キロを霧で包む（2時間）',
      2: '視界内に最大10メートルの大きさの幻影を生み出す',
      3: '上記の幻影を動かしたり喋らせたりできる',
      4: '気力×3回消耗。1日一回。視界内のモンスター1体の幻影を生み出す(6時間) 敵を襲わせることができる',
      5: '気力×3回消耗。1日一回。ランク4の効果を装甲に対して及ぼせる',
    }},
];
  ...
  collectForm() {
    ...
    for (const row of f.querySelectorAll('#singer-song-item-tbody tr')) {
      ...
      song_items.push({
        ...
        hit_mod:      parseInt(row.querySelector('[name=ssi_hit_mod]')?.value) || 0,
        damage_bonus: row.querySelector('[name=ssi_damage_bonus]')?.value?.trim() || '',
      });
    }
